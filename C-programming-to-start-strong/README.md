...
컴파일러가 소스코드를 컴파일하기에 앞서 선행처리하는 구문입니다. 모두 `#` 기호가 앞에 붙어있다는 점이 특징입니다. 전처리기로 정의된 것들을 소스코드에 반영한 후에 컴파일합니다. 경우에 따라서 일부 코드를 생략하거나 반대로 다른 코드를 합쳐서 컴파일을 진행합니다.

### `#include`

초반에도 나왔던 예지입니다. 전처리기의 기능은 이름처럼 외부 파일을 현재 소스코드의 위쪽에 포함시켜 컴파일하도록 합니다. 

`#include <stdio.h>`이라는 구문은 소스코드를 컴파일하기에 앞서 `stdio.h` 헤더 파일을 소스에 포함시켜 컴파일할 것을 지시하는 의미입니다.

보통 해더 파일은 어떤 외부 라이브러리의 짝으로 함께 제공되는 것이 많은데, 지금 예를 든 `stdio.h`도 해당합니다. 그리고 파일명 뒤에 `<>`은 헤더파일을 검색하는 위치와 관련이 있습니다. 

직접 만든 헤더파일이 아니라면 보통 헤더파일은 등록된 위치에 모두 모여있습니다. `<>`으로 감싼 헤더파일은 헤더파일 찾을 때 시스템에 등록된 경로에서 찾으라는 의미입니다. 못찾으면 에러가 발생하고 컴파일은 실패합니다. 참고로 시스템 등록은 설치 시점에 합니다.

`""`으로 감싼 경우는 프로젝트의 현재 경로에서 헤더파일을 검색합니다. 보통 해더파일은 시스템 경로에 없고 소스코드와 같은 경로에 있을 때가 많습니다.

### 매크로

매크로(macro)는 외형상 함수이지만 실제로는 함수가 아니라 한행 여러 행으로 기술할 수 있는 구문입니다. 프로그램 코드 중 불연속적으로 반복되는 것을 함수로 만듭니다. 그래야 유지보수 하기 좋습니다. 그런데 자주 호출되는 함수면서 길이가 짧다면 함수로 만드는 것이 효율적인지 생각해봐야 합니다.

함수를 호출하려면 스택 프레임에 매개변수 메모리도 지정해야 하고 인수도 복사해야 하고 제어의 흐름도 변경해야 합니다. 정작 함수로 수행할 코드는 얼마 안됩니다. 함수 호출 오버헤드가 커질 수 있습니다. 배보다 배꼽이 더 큰 상황이 될 수 있습니다. 

더 좋은 것은 매크로와 `__inline`입니다. 순서를 따지면 매크로가 구형이고 `__inline`이 신형입니다. 매크로의 단점이 보완되었습니다. 물론 레거시스템을 다루는 능력이 진짜 실력입니다. 모두 아는 것이 당연합니다.

#### `__inline` 함수와 매크로


릴리즈 모드로 빌드하면 자동으로 인라인 확장될 것입니다. 릴리즈모드로 놓고 보면 함수로 만드나 맥크로로 만드나 크게 차이가 없어 보입니다.
...
윈도우 개발자는 일반함수와 매크로를 구별하기 위해 매크로는 모두 대문자로 기술하는 경향이 있습니다. 강제는 아닙니다.

어셈블리 코드에 `Add` 함수는 `call` 명령이 보입니다.

`ADD` 매크로는 `3`과 `4`를 더한 `7`을 함수의 매개변수로 전달하는 코드만 보입니다. 함수 호출연산을 하지 않고 그로 인해 오버해드도 없습니다. 인라인 함수와 매크로가 달성하고자 하는 목표입니다. 함수의 장점은 유지하고 성능 향상시키는 것입니다.

매크로를 정의하는 `#define ADD(a, b) (a + b)`을 보면 코드 내부 구문이 괄호로 묶여 있습니다. 문법으로 강제된 것은 아닙니다. 하지만 대부분 매크로는 괄호가 존재합니다. 이유는 매크로가 함수가 아니기 때문입니다. 괄호가 없으면 매크로 연산이 온전히 한 덩어리로 취급받지 못합니다. 이런 사실에도 불구하고 외형상으로는 함수의 모습을 하고 있습니다. 때문에 예기치 못한 오류가 발생하기도 합니다.
...
지금은 괄호가 있어서 제대로 출력되어 있습니다. 하지만 만약에 없었다면 `11`이 출력되었을 것입니다.

#### 매크로 특수화 연산자 `#`, `##`

다음은 매크로 특수화 연산자입니다. C 언어는 일반적인 연산자에 `#`과 `##`은 없습니다. 두 연산자는 오로지 매크로 내부에서만 사용할 수 있는 연산자입니다. `#`은 인자가 무엇인든 모두 만자열로 만들어줍니다. `##`은 2개의 매개변수를 한 덩어리로 묶어 코드를 만들어 주는 연산자입니다.
...
`PASTER` 매크로는 `2`개의 토큰을 인자로 받아 새로운 하나의 토큰으로 만들어줍니다. 문자열이 이 아니라 실제 코드가 그렇게 됩니다. 컴파일러가 구문을 분석할 때 대상요소 하나를 지칭하는 토큰이라고 한 것입니다. 그래서 `printf("%d\n", nData);`으로 번역하게 됩니다. 

이와 같은 매크로를 잘 알아두면 코드 생성기 혹은 코드 자동화를 구현하는데 도움 됩니다. 매크로라는 것의 가장 큰 쓰임새 중 하나는 바로 코드 생성기를 이용한 자동화에 있습니다. 기술적으로 함수 호출의 오버헤드를 줄이는 역할도 있습니다. 맥크로를 이용해 특정 소스코드를 생성하도록 할 수 있습니다. 여기에 조건부 컴파일 전처리까지 더하면 매우 정교한 처리가 가능합니다.

### 조건부 컴파일

상수 정의에 따라 실제로 번역되는 소스코드가 달라지도록 수성하는 것입니다. `#if 조건`, `#else`, `#endif` 이렇게 3가지 전처리기를 사용해서 두 덩어리 코드 중 하나만 선택해 배타적으로 컴파일할 수 있습니다. 다음 예제는 조건부 컴파일 문법을 확인하기 위한 예제입니다.
...
`_MSGTEST_` 상수가 정의되어 있었다면 다른 조건을 탓을 것입니다. 전처리에 다른 문자열이 박혔을 것입니다. `#define _MSGTEST_` 이문구만 조건 전에 선언하면 결과가 달라질 것입니다.

조건부 컴파일을 이용하여 할 수 있는 가장 대표적인 것 중 하나는 문자열 처리와 관련된 것들입니다. MBCS와 유니코드 문자집합을 모두 지원해야 하면 상황에 따라 문자형에 대한 정의를 변경해 프로그램을 만들어 갈 수 있습니다.
...
이런식으로 조건에 따라 호출하는 것을 달라지게 만들 수 있습니다.

이와 같이 조건부 컴파일이 적용되는 사례에는 빌드 모드도 있습니다. 디버그 릴리즈 모드 차이 때문에 전혀 다른 코드로 번역될 수 있게 코드를 구성합니다. 
...
이 코드를 잘 알아 두어야 할 것은 실제로 개발자들이 프로그램을 디버깅할 때 많이 사용하는 코드입니다. 어떤 정보가 화면에 출력될 때, 개발자는 많은 정보를 보아야 하지만 사용자는 굳이 그럴 이유는 없습니다. 이럴 때 조건부로 컴파일 전처리기로 구성해서 2가지 코드를 작성하고 디버깅은 개발자용 메시지 릴리즈는 일반 사용자용 메시지를 출력하게 만들 수 있습니다.

자주 사용나는 전처리기인 `#pragma` 전처리기는 나중에 꼭 다룰 것입니다. 구조체에 이미 다루었지만 중요한 개념이라 다음 장에 다룹니다. 

1. 다음 매크로 정의 문제점은 무엇인가?

```c
#define ADD(a, b) a + b
```

컴파일 과정에서 `printf("%d\n", ADD(3, 4) * 2);`처럼 호출했다면 전처리 시점에는  `printf("%d\n", 3 + 4 * 2);` 이 됩니다. 보통 합산이 의도일 텐데 그전에 연산 우선순위 때문에 실행할 때는 곱셈을 먼저 처리하고 다음에 덧셈을 처리해서 의도와 다르게 14가 아닌 11이 출력될 것입니다.

2. `__inline` 함수와 매크로 함수의 공통점은 무엇인가?
